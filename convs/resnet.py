...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        elif 'cub200' in args["dataset"] or 'cars' in args["dataset"]:
            self.conv1 = nn.Sequential(
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        state_dict_ = {k: v for k, v in state_dict.items() if k not in ['fc.weight', 'fc.bias', 'bn1.running_var', 'bn1.bias', 'bn1.weight', 'bn1.running_mean', 'conv1.weight']}  # needed for cosine incremental fc layer

        # Since this code here has a sequential block in conv1, so, we have to change the keys to load the pretrained imagenet weights
        state_dict_['conv1.1.running_var'] = state_dict['bn1.running_var']
        state_dict_['conv1.1.weight'] = state_dict['bn1.weight']
        state_dict_['conv1.1.bias'] = state_dict['bn1.bias']
        state_dict_['conv1.1.running_mean'] = state_dict['bn1.running_mean']
        state_dict_['conv1.0.weight'] = state_dict['conv1.weight']
        model_dict.update(state_dict_)
