...
@torch.no_grad()
def get_representation_matrix_ResNet18(model, device, x, y=None):
    # Nếu model là CosineIncrementalNet thì lấy backbone convnet
    ...
    b = r[0:100]  # chọn 100 mẫu ngẫu nhiên
    example_data = x[b]
    example_data = example_data.to(device)
    _ = model(example_data)  # forward để kích hoạt hooks

    # Thu thập activations từ hooks
    act_list = [
        model.act["conv_in"],
        model.act["layer1_0_conv1"],
        model.act["layer1_0_conv2"],
        model.act["layer1_1_conv1"],
        model.act["layer1_1_conv2"],
        model.act["layer2_0_conv1"],
        model.act["layer2_0_conv2"],
        model.act["layer2_1_conv1"],
        model.act["layer2_1_conv2"],
        model.act["layer3_0_conv1"],
        model.act["layer3_0_conv2"],
        model.act["layer3_1_conv1"],
        model.act["layer3_1_conv2"],
        model.act["layer4_0_conv1"],
        model.act["layer4_0_conv2"],
        model.act["layer4_1_conv1"],
        model.act["layer4_1_conv2"],
    ]

    # Cấu hình cho ResNet18 (giống paper PGM)
    ...
    in_channel  = [3,64,64,64,64,128,128,128,128,256,256,256,256,512,512,512,512]

    pad = 1
    sc_list = [5,9,13]  # shortcut layers
    p1d = (1,1,1,1)
    ...
    for i in range(len(stride_list)):
        ...
        s = (map_list[i] + 2*pad - ksz)//st + 1  # compute_conv_output_size
